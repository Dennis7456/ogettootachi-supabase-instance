...
export default async function addTestDocument(client, _index) {
  const testDocument = {
    ...
    content:
      'Detailed overview of patent application requirements and processes in Kenya',
  ...
  try {
    const { _data, _error } = await client
      ...
      .select();
    if (_error) {
      console.error('Error adding document:', _error);
    } else {
    }
  } catch (_error) {
    console.error('Failed to add document:', _error);
  }
}
