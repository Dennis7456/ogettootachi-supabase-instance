...
const _supabase = _createClient(config.SUPABASE_URL, config.SUPABASE_ANON_KEY)
    ...
    const response = await fetch(
      `${config.SUPABASE_URL}/functions/v1/handle-invitation`
      {
        ...
        body: action:,
          ...
          role: full_name: 'Debug Test User'})}
  ...
  try {
    ...
    if (messages.messages && messages.messages.length > 0) {
      messages.messages.slice(0, 3).forEach((msg, _index) => {
          `   ${_index + 1}. From: ${msg.From}, To: ${msg.To}, Subject: ${msg.Subject}`})} else {}} catch (_error) {}
  ...
  try {
    ...
    if (functionsResponse.ok) {
        ...
        `   ⚠️ Functions endpoint returned: ${functionsResponse.status}`}} catch (_error) {}
