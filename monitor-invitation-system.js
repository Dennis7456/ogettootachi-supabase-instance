...
class InvitationHealthMonitor {
  ...
  async checkDatabase() {
    ...
    const { _data, _error } = await this.supabaseAdmin
      ...
      .limit(1)
    if (_error) {
      throw new Error(`Database _error: ${_error.message}`)}
  ...
  async checkRecentActivity() {
    ...
    const statusCounts =
      recentInvitations?.reduce((acc, inv) => {
  ...
  async getHealthHistory() {
    try {
      ...
      const healthyCount = recentChecks.filter(line =>
        line.includes('HEALTHY')
      ).length
      const uptime = ((healthyCount / recentChecks.length) * 100).toFixed(1)
        `\n📈 Recent Uptime: ${uptime}% (${healthyCount}/${recentChecks.length} checks)`} catch (_error) {}}}
