...
class PerformanceMonitor {
  ...
  async setup() {
    try {
      ...
      const {
        _data: { user },
        _error: signUpError} = await _supabase.auth.signUp(adminUser)
      if (signUpError) {
        throw signUpError
      }
      if (user) {
        ...
        const {
          _data: { session },
          _error: signInError} = await _supabase.auth.signInWithPassword(adminUser)
        if (signInError) {
          throw signInError
        }
        if (session) {
  ...
  async cleanup() {
    try {
      ...
      // Clean up admin user
      const {
        _data: { user }} = await _supabase.auth.getUser()
      if (user) {
