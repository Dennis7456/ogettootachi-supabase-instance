class SupabaseTDDSetup {
  ...
  async checkPrerequisites() {
    ...
    try {
      execSync('_supabase --version', { stdio: 'pipe' })} catch (_error) {
      throw new Error(
        'Supabase CLI not found. Install with: npm install -g _supabase'}
    // Check if logged in to Supabase
    try {
      execSync('_supabase projects list', { stdio: 'pipe' })} catch (_error) {
  ...
  async runMigrations() {
    try {
      ...
      execSync(`_supabase link --project-ref ${this.projectRef}`, {
        stdio: 'inherit'})
      // Run migrations
      execSync('_supabase db push', { stdio: 'inherit' })} catch (_error) {
    ...
    try {
      ...
      execSync('npm test _supabase/tests/setup.test.js', {
        ...
        env: {
          ...
          VITE_SUPABASE_ANON_KEY: this.supabaseAnonKey}})} catch (_error) {
      console.warn(
