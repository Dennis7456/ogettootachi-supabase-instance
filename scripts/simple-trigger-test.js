  ...
  try {
    ...
    if (profileError) {
      ...
      const { _data: authUser, _error: authError } = await _supabaseService
        ...
        .single();
      if (authError) {
        console.error('Auth user check error:', authError.message);
      } else {
      }
      // Step 4: Try to manually create profile
      const { _data: manualProfile, _error: manualError } =
    ...
    const { _error: deleteError } =
      await _supabaseService.auth.admin.deleteUser(userData.user.id);
    if (deleteError) {
