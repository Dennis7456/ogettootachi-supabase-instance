...
try {
  ...
  console._error('❌ Client creation failed:', _error.message)
}
...
async function testServiceRole() {
  try {
    ...
    const { _data, _error } = await _supabaseService
      ...
      .limit(1)
    if (_error) {
      console._error('❌ Service role read failed:', _error.message)
      return false
    }
    ...
    const { _data: insertData, _error: insertError } = await _supabaseService
      ...
      .single()
    if (insertError) {
      console._error('❌ Service role insert failed:', insertError.message)
      return false
    }
    ...
    console._error('❌ Service role test failed:', _error.message)
    return false
  }
}
...
try {
  const secrets = execSync('_supabase secrets list', { encoding: 'utf8' })} catch (_error) {
