...
function createErrorResponse(error: Error, statusCode: number = 400, metadata?: Record<string, any>) {
  log({
...
serve(async (req) => {
  ...
  if (req.method === 'POST') {
    try {
      const { name, email, phone, practice_area, preferred_date, preferred_time, message } = await req.json()
      
      // Validate required fields
      if (!name || !email || !practice_area || !preferred_date || !preferred_time) {
        throw new Error('Missing required fields: name, email, practice_area, preferred_date, preferred_time')
      }
