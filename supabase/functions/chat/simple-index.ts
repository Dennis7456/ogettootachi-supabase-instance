...
async function searchDocuments(queryEmbedding: number[], topK: number): Promise<Array<{ id: string; content: string }>> {
  ...
  try {
    console.log('Searching documents', { topK, embeddingLength: queryEmbedding.length })
...
async function handleChatRequest(req: Request): Promise<Response> {
  try {
    ...
    if (!body.topK || typeof body.topK !== 'number' || body.topK < 1 || body.topK > 20) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: 'topK must be a number between 1 and 20' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    
    console.log('Processing chat request', { queryLength: body.query.length, topK: body.topK })
