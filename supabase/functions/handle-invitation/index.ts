...
try {
  ...
  console.error('Database error checking for existing invitation:', error);
  return createErrorResponse(new Error('Failed to check for existing invitation'), {
    method,
    path,
    statusCode: 500,
    details: 'Database error checking for existing invitation'
  });
}
...
return new Response(
  ...
  return createErrorResponse(
    new Error(`Failed to process invitation: ${errorMessage}`),
    {
      method: req.method,
      path: req.url,
      statusCode: 500,
      metadata: { error: 'Failed to process invitation' }
    }
  );
}
