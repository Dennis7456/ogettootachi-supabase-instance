-- Fix accept_user_invitation function to work properly with Supabase Auth
-- This migration uses the correct approach for user creation in Supabase
...
BEGIN
  ...
  ELSE
    ...
    -- The actual user will be created through Supabase Auth in the frontend
    new_user_id := gen_random_uuid();
    
    -- Create user profile (this will be updated when user is created via auth)
    INSERT INTO profiles (
      id,
      full_name,
      role,
    ...
    SELECT jsonb_build_object(
      ...
      'email', invitation_record.email,
      'role', invitation_record.role,
      'message', 'Profile created, please create your account with the provided credentials'
    ) INTO result;
