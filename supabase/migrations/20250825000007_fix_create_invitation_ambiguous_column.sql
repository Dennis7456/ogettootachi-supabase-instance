-- Fix create_user_invitation function to resolve ambiguous column reference
-- This migration fixes the 'column reference "expires_at" is ambiguous' error
...
BEGIN
  ...
  calculated_expires_at := NOW() + (expires_in_hours || ' hours')::INTERVAL;
  ...
  -- Return invitation details using the calculated value
