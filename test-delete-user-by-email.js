...
async function testDeleteUserByEmail() {
  console.log('Testing user deletion by email...');

  try {
    // Step 1: Create a test user
    const timestamp = new Date().getTime();
    const userEmail = `test-delete-email-${timestamp}@example.com`;
    const userData = {
      email: userEmail,
    ...
    console.log('Deleting user using delete_user_by_email function...');
    ...
    console.log('✅ User deletion by email test completed successfully');
    return true;
  } catch (error) {
