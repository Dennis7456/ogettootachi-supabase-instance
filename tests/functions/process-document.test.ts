...
describe('Process Document Edge Function', () => {
  ...
  beforeAll(async () => {
    ...
    if (user) {
      ...
      while (!session && attempts < 3) {
        const { data: { session: signInSession }, error: signInError } = await supabase.auth.signInWithPassword(adminUser)
        if (signInError) {
  ...
  describe('POST /functions/v1/process-document', () => {
    ...
    it('should reject request from non-admin users', async () => {
      ...
      const { data: { user }, error: signUpError } = await supabase.auth.signUp(regularUser)
      ...
      if (user) {
        ...
        const { data: { session }, error: signInError } = await supabase.auth.signInWithPassword(regularUser)
