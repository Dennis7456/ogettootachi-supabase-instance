...
const SUPABASE_ANON_KEY = env.SUPABASE_ANON_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6ImFub24ifQ.625_WdcF3KHqz5i_FZUmjo6K0nsL4wYloRmk67f1E7c";
const FRONTEND_URL = env.FRONTEND_URL || "http://localhost:5173";
const SUPABASE_SERVICE_ROLE_KEY = env.SUPABASE_SERVICE_ROLE_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6InNlcnZpY2Vfcm9sZSJ9.vI9P_TzQz0t4q3YZbvD4yCYNuJiMZ-Y5zWyqFQCSrrs";
...
Deno.test("User Invitation Flow", async (t) => {
  ...
  try {
    ...
    await t.step("Send initial invitation", async () => {
      ...
      try {
        const response = await Promise.race([fetchPromise, timeoutPromise]) as CustomFetchResponse;
        ...
        assertStringIncludes(responseData.message || '', "Invitation sent", "Incorrect success message");
      } catch (error) {
    ...
    await t.step("Prevent duplicate invitation", async () => {
      ...
      assertStringIncludes(responseData.error || '', "already exists", "Incorrect error message for duplicate invitation");
    });
    ...
    await t.step("Force resend invitation", async () => {
      ...
      assertStringIncludes(responseData.message || '', "Invitation resent", "Incorrect success message for resend");
    });
